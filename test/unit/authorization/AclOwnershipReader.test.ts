...
import { AclOwnershipReader } from '../../../src/authorization/AclOwnershipReader';
import type {
  AccountSettings,
  AccountStore,
...
describe('An AclOwnershipReader', (): void => {
  ...
  beforeEach(async(): Promise<void> => {
    ...
    reader = new AclOwnershipReader(accountStore, aclStrategy);
  });
  ...
  it('returns empty permissions if the target identifier is not in the pod.', async(): Promise<void> => {
    identifier.path = 'http://somewhere.else/';
    await expect(reader.handle({ credentials, identifier })).resolves.toEqual({});
  });
