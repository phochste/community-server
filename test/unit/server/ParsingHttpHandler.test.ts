...
describe('A ParsingHttpHandler', (): void => {
  ...
  let operationMetadataCollector: jest.Mocked<OperationMetadataCollector>;
  ...
  beforeEach(async(): Promise<void> => {
    ...
    handler = new ParsingHttpHandler(
      { requestParser, operationMetadataCollector, errorHandler, responseWriter, source },
    );
  });
  ...
  it('calls the operation metadata collector if there is response metadata.', async(): Promise<void> => {
    ...
    await expect(handler.handle({ request, response })).resolves.toBeUndefined();
    expect(operationMetadataCollector.handleSafe).toHaveBeenCalledTimes(1);
    expect(operationMetadataCollector.handleSafe).toHaveBeenLastCalledWith({ operation, metadata });
  });
